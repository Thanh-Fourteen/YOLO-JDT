"""Run YOLO11-JDE (JointHead) + BoT-SORT-ReID over a standard-format dataset
split, emit MOT-format prediction txts ready for TrackEval.

Differences from `infer_tracking.py`:
- Loads a JointHead-equipped model (YOLO11 + cv4 ReID branch).
- After NMS, extracts the ReID embedding for each surviving detection
  by indexing into the per-level reid feature maps via the surviving
  anchor's global index.
- Uses BoT-SORT-ReID tracker with the IoU + cosine cost matrix.
...
Usage:
    python -m yolo_jdt.scripts.infer_tracking_jde \\
        ...
        --output-dir runs/baselines/step4_yolo11s_botsort_reid_mot17/tracker_outputs \\
        --conf 0.05
"""
...
def load_jde_model(weights_path: Path, scale: str, device: torch.device):
    """Load a promoted JDE checkpoint into _YOLO11WithJointHead."""
    ...
    if isinstance(payload, dict) and "state_dict" in payload:
        ...
        sd, nc = payload, 1
    model = _YOLO11WithJointHead(scale=scale, nc=nc).to(device).eval()
    result = model.load_state_dict(sd, strict=False)
    if result.missing_keys:
        print(f"[infer_jde] WARN missing {len(result.missing_keys)} keys: "
              f"{result.missing_keys[:5]}")
    if result.unexpected_keys:
        print(f"[infer_jde] WARN unexpected {len(result.unexpected_keys)} keys: "
...
def _nms_with_anchor_idx(decoded: torch.Tensor, conf_thr: float = 0.05,
    ...
    """Same semantics as `_multi_label_nms` for nc=1 (person), but also
    returns the original anchor index for each surviving detection so
    the caller can gather ReID embeddings.

    Returns per-image dict: {boxes_xyxy, scores, labels, anchor_idx}.
    """
    bs, ch, A = decoded.shape
    nc = ch - 4
    assert nc == 1, "infer_tracking_jde currently only supports nc=1 (person-only)"
    ...
    for i in range(bs):
        ...
        boxes_xywh = x[:, :4]
        scores = x[:, 4]
        keep_mask = scores > conf_thr          # [A] bool
...
def infer_one_seq(model, device, dtype, json_path: Path, image_root: Path,
    ...
    """Run JDE pipeline over one sequence."""
    ...
    for fi, frame in enumerate(seq["frames"]):
        ...
        with torch.no_grad():
            decoded, _, reid_per_level = model(x)
        if device.type == "cuda":
        ...
        anchor_idx = preds["anchor_idx"]                # tensor on device

        # Gather ReID embeddings for surviving anchors
        if len(boxes_xyxy_n) > 0:
            # Flatten reid per-level into [reid_dim, A] on device
            reid_flat = torch.cat(
                [r.view(r.shape[1], -1) for r in (rr[0] for rr in reid_per_level)],
                dim=1,
            ) if False else torch.cat(
                # explicit form: each reid_per_level[i] is [B, 128, H, W]; bs=1
                [reid_per_level[lvl][0].view(reid_per_level[lvl].shape[1], -1)
                 for lvl in range(len(reid_per_level))],
                dim=1,
            )                                            # [reid_dim, A]
            embs = reid_flat[:, anchor_idx].T.float().cpu().numpy()   # [N, reid_dim]
...
def main():
    ...
    ap.add_argument("--tracker", default="botsort_reid",
                    choices=["botsort_reid"],
                    help="Currently only BoT-SORT-ReID supported (uses cv4 embeddings)")
    ...
    print(f"[infer_jde] device={device}, dtype={dtype}, tracker={args.tracker}")
    print(f"[infer_jde] dataset={args.dataset}/{args.split}, weights={args.weights}")

    args.output_dir.mkdir(parents=True, exist_ok=True)

    model, nc = load_jde_model(args.weights, args.scale, device)
    model = model.to(dtype)
    print(f"[infer_jde] loaded YOLO11{args.scale}-JDE, nc={nc}, "
    ...
    print(f"\n[infer_jde] wrote {fps_out}")
    print(f"[infer_jde] DONE")
