#!/usr/bin/env bash
# Resume từ Stage B (Stage A đã xong: epoch=000-stepstep=2207-v1.ckpt)
# Chạy: Stage B → promote → eval → tracking x3 → Phase 2 YOLO11m 70ep
set -u
...
STAGE_A_CKPT="runs/jde/step4_jde_yolo11s_stageA/epoch=000-stepstep=2207-v1.ckpt"

echo "[$(date +%H:%M:%S)] ============================================"
echo "[$(date +%H:%M:%S)]  Resume: Stage B → promote → eval → tracking → YOLO11m"
echo "[$(date +%H:%M:%S)] ============================================"
echo "[$(date +%H:%M:%S)] Stage A ckpt: $STAGE_A_CKPT"
