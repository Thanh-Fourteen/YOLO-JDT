"""Hydra entry point for YOLO11-JDE 2-stage training (Step 4 / Phase 4).

Usage:
    # Stage A — freeze backbone+neck, train head + ReID classifier ~5 epochs
    python -m yolo_jdt.scripts.train_jde -cn jde \\
        run_name=jde_yolo11s_stageA model.stage=A trainer.max_epochs=5 \\
        model.pretrained_weights=weights/ours/yolo11s_det.pt

    # Stage B — unfreeze, fine-tune at LR/10 ~15 epochs
    python -m yolo_jdt.scripts.train_jde -cn jde \\
        run_name=jde_yolo11s_stageB model.stage=B trainer.max_epochs=15 \\
        model.pretrained_weights=runs/jde/jde_yolo11s_stageA/last.ckpt
"""
...
from yolo_jdt.train.jde_datamodule import JDEDataModule
from yolo_jdt.train.jde_lightning_module import JDELitModule


class CloseMosaicCallback(L.Callback):
    """Disable mosaic in the final `1 - fraction` epochs."""
    def __init__(self, fraction: float = 0.85, max_epochs: int = 5):
        super().__init__()
...
@hydra.main(config_path="../configs", config_name="jde", version_base="1.3")
def main(cfg: DictConfig):
    ...
    # Resolve paths absolute (Hydra changes cwd)
    ...
    # DataModule first — to discover num_track_ids
    dm = JDEDataModule(
    ...
    dm.setup()
    auto_num_ids = dm.num_track_ids
    print(f"[train_jde] datamodule reports num_track_ids = {auto_num_ids}")

    # Resolve auto num_track_ids
    num_track_ids = (auto_num_ids if cfg.model.num_track_ids in (-1, None)
